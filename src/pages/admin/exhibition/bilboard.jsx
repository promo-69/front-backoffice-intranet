        ...
        <div className="flex items-center justify-between px-4 py-3 bg-white border-t border-gray-200 sm:px-6 rounded-b-xl shadow-sm">
          <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
            <p className="text-sm text-gray-700">
              Mostrando {(currentPage - 1) * itemsPerPage + 1} a {Math.min(currentPage * itemsPerPage, movies.length)} de {movies.length} resultados
            </p>
          ...
          {isLoading ? (
            skeletonRows.map((_, index) => (
              <tr key={`skeleton-${index}`} className="animate-pulse">
                ...
                <td className="py-4 px-6"><div className="flex justify-center gap-2"><div className="w-7 h-7 bg-slate-200 rounded"></div><div className="w-7 h-7 bg-slate-200 rounded"></div></div></td>
              </tr>
            ))
          ) : data && data.length > 0 ? (
            data.map((movie) => (
              <tr key={movie.id} className="hover:bg-brand-primary/5 transition-colors group">
                ...
                <td className="py-4 px-4 text-center"><span className="px-2 py-1 rounded bg-brand-primary/5 text-brand-primary font-black text-[10px] border border-brand-primary/10 whitespace-nowrap">{movie.age_classification?.description || CLASSIFICATION_FALLBACK[movie.age_classification] || "N/A"}</span></td>
            ...
            <tr><td colSpan="8" className="text-center py-12 text-slate-400 font-montserrat uppercase text-[10px] tracking-widest font-bold">No hay películas disponibles</td></tr>
          )}
        </tbody>
      </table>
    </div>
