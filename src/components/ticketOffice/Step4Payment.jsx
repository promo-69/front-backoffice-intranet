...
export default function Step4Payment({ movie, showtime, selectedSeats = [], ticketsNeeded = 1, totalTickets = 0, totalTicketsVes = 0, concessionItems = [], concessionTotal = 0, concessionTotalVes = 0, onConfirm, onBack, onNewSale, paymentMethods = [], bankAccountsByMethod = {}, vesCurrencyId = 2, customerInfo = null, exchangeRate = 600 }) {
  ...
  const [payments, setPayments] = useState([{ method: defaultMethod, amountVes: initAmountVes, amountUsd: initAmountUsd, fields: {} }]);
  const [confirmed, setConfirmed] = useState(false);

  const grandTotal = totalTickets + concessionTotal;
  const grandTotalVes = totalTicketsVes + concessionTotalVes;
  const paidByUser = payments.reduce((s, p) => s + (Number(p.amountUsd) || 0), 0);
  ...
  const addPayment = () => {
    ...
    setPayments([...payments, { method: nextMethod?.id || 1, amountVes: 0, amountUsd: 0, fields: {} }]);
  };
  ...
  const onAmountUsdChange = (index, usdAmount) => {
    ...
    updatePayment(index, { amountUsd: usd, amountVes: Math.round(usd * exchangeRate * 100) / 100 });
  };

  const onAmountVesChange = (index, vesAmount) => {
    const ves = parseFloat(vesAmount) || 0;
    updatePayment(index, { amountVes: ves, amountUsd: Math.round((ves / exchangeRate) * 100) / 100 });
  };
  ...
  if (confirmed) {
        ...
        <div className="mt-6 bg-gray-50 border border-[#3E2186]/30 rounded-2xl p-6 text-center w-full max-w-sm">
          {movie && (
            <>
              ...
              <p className="text-slate-800 font-bold">{showtime.time} · {showtime.room}</p>
              <div className="flex gap-2 flex-wrap justify-center mt-2">
                {selectedSeats.map((s) => (
                  <span key={s.id} className="bg-[#3E2186] text-white px-2.5 py-1 rounded-lg font-bold text-xs">{s.id}</span>
                ))}
              </div>
            </>
          )}
          {concessionItems.length > 0 && (
            <div className="mt-2 space-y-1">
              {concessionItems.map((entry, idx) => (
                <p key={entry.id || entry._key || idx} className="text-sm text-slate-700">{entry.item?.name || entry.name} ×{entry.qty ?? entry.quantity}</p>
              ))}
            </div>
          )}
          <p className="text-2xl font-bold text-[#3E2186] mt-4">${grandTotal.toFixed(2)}</p>
            ...
            {payments.map((p, i) => {
              ...
              return (
                <p key={i} className="text-xs text-slate-700">
                  ...
                  {p.amountVes > 0 && <span className="text-slate-600"> · Bs. {(Number(p.amountVes) || 0).toFixed(2)}</span>}
                </p>
              );
            })}
            </div>
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
      <div>
        ...
        <p className="text-slate-600 text-sm mt-1">Confirma los detalles y selecciona los métodos de pago</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-2xl p-5 space-y-4">
            ...
            <>
              ...
              <div className="space-y-1.5 text-sm">
                <div className="flex justify-between text-slate-700">
                  ...
                  <span className="text-slate-800 font-bold">{ticketsNeeded}x</span>
                </div>
                ...
                <div className="flex justify-between font-bold pt-1 border-t border-gray-200">
                  ...
                  <div className="text-right">
                    <span className="text-[#3E2186]">${totalTickets.toFixed(2)}</span>
                    {totalTicketsVes > 0 && <span className="text-slate-600 text-xs font-normal block">Bs. {totalTicketsVes.toFixed(2)}</span>}
                  </div>
                </div>
              </div>
            </>
          ...
          {concessionItems.length > 0 && (
            <>
              ...
              <div className="space-y-1">
                {concessionItems.map((entry, idx) => (
                  <div key={entry._key || entry.id || idx} className="flex justify-between text-sm text-slate-700">
                    <span>{entry.item?.emoji || ""} {entry.item?.name || entry.name} ×{entry.qty ?? entry.quantity}</span>
                    <span>${((entry.item?.price ?? entry.price) * (entry.qty ?? entry.quantity)).toFixed(2)}</span>
                  </div>
                ))}
                <div className="flex justify-between font-bold pt-1 border-t border-gray-200">
                  <span className="text-slate-700">Subtotal confitería</span>
                  <div className="text-right">
                    <span className="text-[#3E2186]">${concessionTotal.toFixed(2)}</span>
                    {concessionTotalVes > 0 && <span className="text-slate-600 text-xs font-normal block">Bs. {concessionTotalVes.toFixed(2)}</span>}
                  </div>
                </div>
              </div>
            </>
          ...
          <div className="flex justify-between text-xl font-bold pt-3 border-t-2 border-[#3E2186]/30">
            ...
            <div className="text-right">
              <span className="text-[#3E2186] block">${grandTotal.toFixed(2)}</span>
              {grandTotalVes > 0 && <span className="text-slate-600 text-sm font-normal">Bs. {grandTotalVes.toFixed(2)}</span>}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <h3 className="font-bold text-[#3E2186] text-sm uppercase tracking-wider">Métodos de Pago</h3>

          {paymentMethods.length === 0 ? (
            <p className="text-slate-600 text-sm text-center py-8">Cargando métodos de pago...</p>
          ) : (
                ...
                return (
                  <div key={index} className="bg-gray-50 border border-gray-200 rounded-xl p-4 space-y-3">
                    <div className="flex items-center justify-between gap-2">
                      <select
                        value={p.method}
                        onChange={(e) => { const newMethod = Number(e.target.value); updatePayment(index, { method: newMethod, amountVes: 0, amountUsd: 0, fields: {} }); }}
                    ...
                    {bankAccounts.length > 0 && (
                      <div className="relative">
                        <label className="absolute top-1 left-3 text-[10px] font-bold text-[#3E2186] uppercase tracking-wider">Banco</label>
                        <select
                          value={p.fields?._baId || ""}
                          onChange={(e) => {
                            const baId = e.target.value;
                            const ba = bankAccounts.find(b => b.id === Number(baId));
                            updatePayment(index, {
                              fields: {
                                ...p.fields,
                                _baId: baId,
                        ...
                        >
                          <option value="" className="text-slate-500">Seleccionar banco</option>
                          {bankAccounts.map((ba) => (
                    ...
                    {p.fields?._baPaymentDetails?.length > 0 && p.fields._baPaymentDetails.map((detail, i) => (
                      <div key={i} className="relative">
                        <label className="absolute top-1 left-3 text-[10px] font-bold text-[#3E2186] uppercase tracking-wider">{detail.label}</label>
                        <input
                          type="text"
                          readOnly
                          value={detail.value}
                    ...
                    <div className="flex gap-2">
                      <div className="relative flex-1">
                        <label className="absolute top-1 left-3 text-[10px] font-bold text-[#3E2186] uppercase tracking-wider">$</label>
                        <input
                          type="number"
                          step="0.01"
                          min="0"
                          value={p.amountUsd || ""}
                          onChange={(e) => onAmountUsdChange(index, e.target.value)}
                      ...
                      <div className="relative flex-1">
                        <label className="absolute top-1 left-3 text-[10px] font-bold text-[#3E2186] uppercase tracking-wider">Bs.</label>
                        <input
                          type="number"
                          step="0.01"
                          min="0"
                          value={p.amountVes || ""}
                          onChange={(e) => onAmountVesChange(index, e.target.value)}
                          className="w-full bg-white border border-gray-200 rounded-xl px-3 pt-6 pb-2 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-[#3E2186]/60 transition-colors"
                        />
                      </div>
                    </div>
                    ...
                    {hasReference && (
                      <div className="relative">
                        <label className="absolute top-1 left-3 text-[10px] font-bold text-[#3E2186] uppercase tracking-wider">Referencia</label>
                        <input
                          type="text"
          ...
          {isOverpaying && (
            <p className="text-sm text-red-500 font-medium text-center">
              Los montos exceden el total por ${(paidByUser - grandTotal).toFixed(2)}
            </p>
          )}
        </div>
      </div>
      ...
      <div className="flex justify-between pt-4">
        ...
        >
          <ArrowLeft className="w-4 h-4" /> {movie ? "Volver" : "Volver a Productos"}
        </button>
