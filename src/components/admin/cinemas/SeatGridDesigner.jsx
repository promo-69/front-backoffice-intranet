...
export default function SeatGridDesigner({ 
  ...
  // true = Modo Estructura
  // false = Modo Categoría
  const [editMode, setEditMode] = useState(true);

  // Evita recargas infinitas del layout inicial
  const hasLoadedInitialLayout = useRef(false);

  /**
   * =========================================================
   * CARGA DEL LAYOUT INICIAL (EDICIÓN)
   * =========================================================
   */
  useEffect(() => {
      if (
        ...
        !hasLoadedInitialLayout.current
      ) {
     ...
     * ==========================================
     * CASO 1:
     * CARGAR LAYOUT EXISTENTE (EDICIÓN)
     * ==========================================
     */
    ...
    externalFormData.rows,
    externalFormData.cols,
    initialLayout
  ]);

  /**
   * =========================================================
   * VALIDACIÓN HACIA EL PADRE
   * =========================================================
   */
  useEffect(() => {
    if (seatMap.length === 0) return;

    const activeSeatsCount = seatMap
      .flat()
      .filter(s => s.type !== 'empty').length;
  ...
  const handleSeatClick = (rowIndex, colIndex) => {
    ...
    if (editMode) {
      ...
      // NORMAL -> MANTENIMIENTO -> PASILLO -> NORMAL
  ...
  const activeSeatsCount = seatMap
    .flat()
    .filter(s => s.type !== 'empty' && s.condition === 1).length;
  ...
  return (
    <div className="mt-6 border border-gray-200 rounded-cineflix p-6 bg-white shadow-sm">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        <div className="flex gap-3">
          ...
          <div className="bg-brand-primary/5 border border-brand-primary/20 px-4 py-1 rounded flex flex-col items-center justify-center">
            <span className="text-[10px] font-bold text-brand-primary uppercase font-montserrat">
              Capacidad Real
            </span>
            ...
            row.map((seat, colIndex) => {
              ...
              return (
                ...
                >
                  ...
                  {isDisability && (
                    <Accessibility className="w-3.5 h-3.5 mt-0.5 animate-in zoom-in duration-300" />
                  )}

                  {isMaintenance && (
                    <Wrench className="w-3.5 h-3.5 mt-0.5 animate-in zoom-in duration-300" />
                  )}
      ...
      <div className="mt-6 flex flex-col items-center gap-4">
        ...
        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest font-montserrat">
          ...
          >
            ...
            <div className="text-left">
              <p className={`text-[11px] font-bold leading-none ${
                editMode ? 'text-brand-primary' : 'text-slate-500'
              }`}>
                Modo Estructura
              </p>
