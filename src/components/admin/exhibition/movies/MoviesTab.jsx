...
// Fallbacks para datos en caso de que la API devuelva valores nulos
...
// Función auxiliar para renderizar los badges de estado
...
export default function MoviesTable({ data, isLoading, onEdit, onDelete }) {
    ...
    <div className="overflow-x-auto bg-surface-container rounded-cineflix border border-border shadow-sm">
      <table className="min-w-full divide-y divide-[#4B2E83]/60">
        ...
        <tbody className="divide-y divide-border text-brand-primary">
          {isLoading ? (
            // ⭐ VISTA EN MODO DE CARGA (SKELETONS)
            ...
            // ⭐ DATOS CONTROLADOS POR EL MANAGER
            data.map((movie) => (
                  ...
                  <div className="flex flex-wrap gap-1 max-w-[200px]">
                    {movie.genres?.map((g) => (
                      <span key={g.id} className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded text-[9px] font-bold uppercase border border-slate-200">
                        {g._Genres?.description || GENRES_FALLBACK[g.genre] || "Otro"}
                      </span>
