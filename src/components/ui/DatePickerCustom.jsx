...
export function DatePickerCustom({ label, value, onChange, placeholder = "dd/mm/aaaa" }) {
  const [open, setOpen]   = useState(false);
  const [viewYear, setVY] = useState(() => { const d = parseDate(value); return d ? d.getFullYear() : new Date().getFullYear(); });
  const [viewMonth, setVM]= useState(() => { const d = parseDate(value); return d ? d.getMonth() : new Date().getMonth(); });
  const wrapRef = useRef(null);

  useEffect(() => {
    const d = parseDate(value);
    if (d) { setVY(d.getFullYear()); setVM(d.getMonth()); }
  }, [value]);
  ...
  const prevMonth = () => viewMonth === 0 ? (setVM(11), setVY(y => y - 1)) : setVM(m => m - 1);
  const nextMonth = () => viewMonth === 11 ? (setVM(0), setVY(y => y + 1)) : setVM(m => m + 1);
  ...
  const isToday    = (d) => d === todayObj.getDate() && viewMonth === todayObj.getMonth() && viewYear === todayObj.getFullYear();
  const isSel      = (d) => selected && d === selected.getDate() && viewMonth === selected.getMonth() && viewYear === selected.getFullYear();

  const pick   = (d) => { onChange(toISODate(new Date(viewYear, viewMonth, d))); setOpen(false); };
  const clear  = (e) => { e.stopPropagation(); onChange(""); };
  const goToday = () => { const t = new Date(); setVY(t.getFullYear()); setVM(t.getMonth()); onChange(toISODate(t)); setOpen(false); };
  ...
  return (
    <div className="relative flex flex-col gap-1" ref={wrapRef}>
      ...
      >
        ...
        {value && (
          <span onClick={clear} className="ml-auto text-muted-foreground hover:text-foreground">
